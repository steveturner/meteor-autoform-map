...
Template.afMap.events({
  'click .js-locate': function(e, t) {
    ...
    if (AutoForm.validateField(formId, "address_line_1") &&
        AutoForm.validateField(formId, "city") &&
        AutoForm.validateField(formId, "postcode") &&
        AutoForm.validateField(formId, "country") && formId) {
      
      var addrString = AutoForm.getFieldValue("address_line_1", formId) + " " +
                       AutoForm.getFieldValue("city", formId) + " " +
                       AutoForm.getFieldValue("country", formId) + " " +
                       AutoForm.getFieldValue("postcode", formId);

      var geocoder = new google.maps.Geocoder();
      var that = this;
      addressMarker(addrString);
      
      function addressMarker(address){
        geocoder.geocode( { 'address': address, 'region': 'uk' }, function(results, status) {
          if (status == google.maps.GeocoderStatus.OK) {
              var result = results[0].geometry.location;
              that.setMarker(that.map, result, that.options.zoom);
              that.map.setCenter(result);
              that.loading.set(false);
          } else {
